...
<div class="bg-white rounded-2xl border border-slate-200 shadow-md hover:shadow-lg transition duration-200 p-5 mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <select id="pricingProductSelect" class="rounded-lg border border-gray-300 focus:border-mint-500 focus:ring-0 px-4 py-2.5 text-sm">
                <option value="">Select a product...</option>
                <?php foreach ($products as $p): ?>
                <option value="<?= $p['id'] ?>"><?= e($p['name']) ?> (₱<?= number_format($p['base_price'], 2) ?>)</option>
                <?php endforeach; ?>
            </select>
            <button type="button" onclick="suggestPrice()" id="suggestPriceBtn" class="bg-mint-600 hover:bg-mint-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 shadow-md hover:shadow-lg transition duration-200 whitespace-nowrap">
                <i data-lucide="sparkles" class="w-4 h-4"></i> Suggest Price
            </button>
        </div>
    </div>
    <div id="priceSuggestionResult" class="hidden mt-4 pt-4 border-t border-slate-200"></div>
</div>
...
function suggestPrice() {
    ...
    result.innerHTML = '<div class="text-center py-4"><i data-lucide="loader-2" class="w-6 h-6 mx-auto mb-2 text-slate-400 animate-spin"></i><p class="text-sm text-slate-500">Analyzing sales data with AI...</p></div>';
    lucide.createIcons();
            ...
            result.innerHTML = `
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="p-4 rounded-xl bg-mint-50 border border-mint-200">
                        ...
                        <p class="text-2xl font-bold text-mint-700 mt-1">₱${Number(s.suggested_price || 0).toFixed(2)}</p>
                    </div>
                ...
                ${s.recommendation ? '<div class="mt-2 p-3 rounded-xl bg-mint-50 border border-mint-200"><p class="text-xs font-medium text-mint-700 uppercase tracking-wider mb-1">Recommendation</p><p class="text-sm text-slate-700">' + s.recommendation + '</p></div>' : ''}
            `;
        }
        lucide.createIcons();
    })
