    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-md hover:shadow-lg transition duration-200 overflow-hidden mb-6">
        ...
        <div class="p-5">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" onclick="aiForecast(<?= $batch['id'] ?>)" id="aiForecastBtn" class="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium inline-flex items-center gap-2 shadow-md hover:shadow-lg transition duration-200">
...
function aiForecast(batchId) {
    ...
    result.innerHTML = '<div class="text-center py-4"><i data-lucide="loader-2" class="w-6 h-6 mx-auto mb-2 text-cyan-400 animate-spin"></i><p class="text-sm text-slate-500">AI is analyzing batch conditions...</p></div>';
    lucide.createIcons();
            ...
            result.innerHTML = `
                <div class="p-4 rounded-xl bg-cyan-50 border border-cyan-200">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <p class="text-xs text-slate-500 font-medium flex items-center gap-1"><i data-lucide="sparkles" class="w-3 h-3 text-cyan-600"></i> AI Projected Ready Date</p>
                            <p class="text-2xl font-bold text-cyan-700 mt-1">${f.projected_ready_date || 'N/A'}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs text-slate-500 font-medium">Days Until Ready</p>
                            <p class="text-2xl font-bold text-cyan-700">${f.days_until_ready ?? 'N/A'}</p>
                        </div>
                    </div>
                    <div class="mb-3">
                        <p class="text-xs text-slate-500 font-medium mb-1">Confidence: ${f.confidence || 0}%</p>
                        <div class="w-full bg-slate-100 rounded-full h-2">
                            <div class="bg-cyan-500 rounded-full h-2" style="width: ${f.confidence || 0}%"></div>
                        </div>
                    </div>
                    ...
                    ${f.recommendation ? '<div class="p-3 rounded-xl bg-white border border-cyan-200"><p class="text-xs font-medium text-cyan-600 uppercase tracking-wider mb-1">Care Recommendation</p><p class="text-sm text-slate-700">' + f.recommendation + '</p></div>' : ''}
                </div>
            `;
        }
        lucide.createIcons();
